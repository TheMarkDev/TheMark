...
unsigned int nStakeMinAge = 60 * 60 * 24 * 1; // 1 day as zero time weight
unsigned int nStakeMaxAge = 60 * 60 * 24 * 45; // 45 days as full weight
...
const string strMessageMagic = "TheMark Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, uint256 prevHash)
{
    ...
    } else if(nHeight >= 10 && nHeight <= 42510){ 
    nSubsidy = 400 * COIN;  
    } else if(nHeight >= 42511 && nHeight <= 92511){
    nSubsidy = 200 * COIN;   
    }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in TheMark
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TheMark", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "FBI and CIA conclusions on Russia hacking don't line up 12-11-16";
        CTransaction txNew;
        txNew.nTime = 1481509283;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print        
        block.print();
        assert(block.hashMerkleRoot == uint256("0xa8afe465fe07570bdb0c425bf0295aa859a60fc7badcd563c13b3eae88fc8453"));
