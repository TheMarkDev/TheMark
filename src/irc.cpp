...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #themarkTEST2\r");
            Send(hSocket, "WHO #themarkTEST2\r");
        } else {
            // randomly join #themark00-#themark05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #themark%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #themark%02d\r", channel_number).c_str());
        }
